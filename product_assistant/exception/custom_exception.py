...
class ProductAssistantException(Exception):
    def __init__(self, error_message, error_details: Optional[object] = None):
        ...
        # Resolve exc_info (supports: sys-like object with exc_info(), an Exception, or current context)
        exc_type = exc_value = exc_tb = None
        if error_details is None:
            exc_type, exc_value, exc_tb = sys.exc_info()
        else:
            if hasattr(error_details, "exc_info"):  # e.g., sys module or a logger passing sys
                exc_type, exc_value, exc_tb = error_details.exc_info()  # type: ignore[attr-defined]
